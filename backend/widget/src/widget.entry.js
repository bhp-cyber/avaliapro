...
/* EFEITO BEHIND THE WALL */
.avaliapro-highlight-is-entering {
  opacity: 0;
  transform: translateY(100%); /* Começa totalmente abaixo */
  clip-path: inset(100% 0 0 0); /* Escondido pela 'parede' */
}

.avaliapro-highlight-is-visible {
  opacity: 1;
  transform: translateY(0);
  clip-path: inset(0 0 0 0);
  /* O segredo da sutileza está neste cubic-bezier quintic */
  transition: opacity 3.5s cubic-bezier(0.05, 1, 0.3, 1), 
              transform 4.0s cubic-bezier(0.16, 1, 0.3, 1), 
              clip-path 4.0s cubic-bezier(0.16, 1, 0.3, 1);
...
>
  ...
  function playHighlightIntro(container) {
    var highlightRotator = container.querySelector(
      "#avaliapro-highlight-rotator"
    );
    if (!highlightRotator) return;

    highlightRotator.classList.remove("avaliapro-highlight-is-visible");
    highlightRotator.classList.add("avaliapro-highlight-is-entering");

    requestAnimationFrame(function () {
      requestAnimationFrame(function () {
        highlightRotator.classList.remove("avaliapro-highlight-is-entering");
        highlightRotator.classList.add("avaliapro-highlight-is-visible");
      });
    });
  }

  function clearHighlightRotationTimer() {
    if (state.highlightRotationTimer) {
      clearTimeout(state.highlightRotationTimer);
      state.highlightRotationTimer = null;
    }
  }

  function scheduleHighlightRotation(container, data, sku, reviews) {
    clearHighlightRotationTimer();
    var list = Array.isArray(reviews) ? reviews : [];
    if (list.length <= 1) return;

    state.highlightRotationTimer = setTimeout(function () {
      ...
      if (modalIsOpen) {
        scheduleHighlightRotation(container, data, sku, reviews);
        return;
      }

      // ALEATORIEDADE: Escolhe um índice aleatório diferente do atual
      var nextIndex;
      do {
        nextIndex = Math.floor(Math.random() * list.length);
      } while (nextIndex === state.activeHighlightIndex && list.length > 1);

      state.activeHighlightIndex = nextIndex;
      renderWidget(container, data, sku);
    }, 5000); // 5 segundos entre cada troca
  }
...
${imageHtml}
  ...
  function renderWidget(container, data, sku) {
    ...
    var highlightSignature = filteredReviews
      .map(function (review) {
        return [
          review && review.id,
    ...
    if (state.lastHighlightSignature !== highlightSignature) {
      state.lastHighlightSignature = highlightSignature;
    ...
    if (filteredReviews.length > 0) {
      highlightIndex = state.activeHighlightIndex % filteredReviews.length;
    }

    var highlightReview = filteredReviews[highlightIndex] || null;

    var highlightText = "";
    var highlightAuthor = "";

    if (highlightReview) {
      var rawHighlightText =
        normalizeText(highlightReview.title) ||
        normalizeText(highlightReview.comment) ||
        "Clientes estão compartilhando experiências positivas sobre este produto.";

      highlightText =
        rawHighlightText.length > 88
          ? safeText(rawHighlightText.slice(0, 85)) + "..."
          : safeText(rawHighlightText);

      highlightAuthor = safeText(
        maskAnonymousName(highlightReview.authorName || "Cliente")
      );
    }

    var highlightCardBaseStyle = `
  display:grid;
  row-gap:10px;
  align-content:start;
  width:100%;
...
`;
  ...
  >
    ...
    >
      <path
        d="M7.5 25.8C11.8 25.8 14.2 22.9 14.2 17.9V10.8C14.2 8.7 12.9 7.4 10.8 7.4H7.9C5.8 7.4 4.5 8.7 4.5 10.8V16.1C4.5 18.2 5.8 19.5 7.9 19.5H10.9C10.8 22.5 9.6 24.4 7.1 25.7"
        stroke="currentColor"
        stroke-width="2.4"
      ...
      />
      <path
        d="M23.3 25.8C27.6 25.8 30 22.9 30 17.9V10.8C30 8.7 28.7 7.4 26.6 7.4H23.7C21.6 7.4 20.3 8.7 20.3 10.8V16.1C20.3 18.2 21.6 19.5 23.7 19.5H26.7C26.6 22.5 25.4 24.4 22.9 25.7"
        stroke="currentColor"
        stroke-width="2.4"
        stroke-linecap="round"
        stroke-linejoin="round"
...
`;
    ...
    var highlightHtml = highlightReview
      ? `
<div style="${highlightCardBaseStyle}">
${highlightQuoteHtml}

<div
id="avaliapro-highlight-rotator"
class="avaliapro-highlight-is-entering"
style="
display:grid;
min-height:96px;
align-content:start;
width:100%;
min-width:0;
overflow:hidden; /* A 'parede' que esconde o texto */
position:relative;
will-change: opacity, transform, clip-path;
"
>
...
width:100%;
max-width:none;
transform:translate(24px,3px); /* Mantém o recuo em relação à aspa */
"
>
${highlightText}
</div>
...
max-width:none;
margin-top:2px;
transform:translate(24px,11px); /* Mantém o recuo em relação à aspa */
"
>
  ${highlightAuthor}
</div>
...
`
    ...
    <div style="${highlightCardBaseStyle}">
      ...
      <div
        ...
        "
      >
        As primeiras experiências dos clientes aparecerão aqui.
      </div>
    </div>
  `;
...
>
  <div style="display:grid;row-gap:8px;min-width:0;justify-self:start;width:max-content;">
    <div style="display:grid;grid-template-columns:auto auto;align-items:center;column-gap:-1px;width:max-content;">
      <span class="avaliapro-average" style="font-size:48px;font-weight:800;letter-spacing:-0.5px;line-height:1;display:flex;align-items:center;align-self:stretch;transform:translate(28px, 3px)">
  ...
  <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;transform:translateX(34px)">
    <span class="avaliapro-stars" style="font-size:8px;line-height:1;transform:translate(-65px, -20px);display:inline-flex;">
  ...
  <div style="display:flex;justify-content:flex-start;align-items:flex-start;width:100%;min-width:0;line-height:0.9;">
  ${highlightHtml}
</div>
  ...
  onmouseenter="
    const arrow=this.querySelector('[data-avaliapro-cta-arrow]');
    const subtitle=this.querySelector('[data-avaliapro-cta-subtitle]');
  ...
  onmouseleave="
    const arrow=this.querySelector('[data-avaliapro-cta-arrow]');
    const subtitle=this.querySelector('[data-avaliapro-cta-subtitle]');
    if(arrow){arrow.style.transform='translateX(0)';arrow.style.opacity='0.9';}
...
>
  ...
  style="
    display:grid;
    grid-template-columns:minmax(0, 340px) 190px;
    align-items:start;
    column-gap:4px;
    width:100%;
    margin-bottom:10px;
...
>
  ...
  style="position:relative;left:6px;top:34px; display:block;"
>
  Seu nome
</label>
      <input
        ...
        required
        style="padding:10px 14px;min-height:18px;margin-top:40px;"
      />
    </div>
...
<div
  id="avaliapro-media-upload-block"
  style="
    display:grid;
    grid-template-columns:minmax(0, 1fr) auto;
    align-items:center;
...
>
  <div style="display:grid;gap:4px;min-width:0;">
    <span style="font-size:14px;font-weight:600;color:#111827;line-height:1.25;">
      Ganhe 5% de desconto ao enviar foto ou vídeo
    </span>
    <span style="font-size:12px;color:#6b7280;line-height:1.25;">
      Opcional • até 3 fotos e 1 vídeo
    </span>
  </div>

    <button
    type="button"
    id="avaliapro-media-upload-trigger"
    style="
      display:inline-flex;
      align-items:center;
      gap:10px;
  ...
  >
    ...
    >
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden="true">
        <path d="M12 5v14M5 12h14" stroke="#111827" stroke-width="1.7" stroke-linecap="round"/>
      </svg>
    </div>
...
>
  ...
  >
    ...
    playHighlightIntro(container);
    scheduleHighlightRotation(container, data, sku, filteredReviews);
